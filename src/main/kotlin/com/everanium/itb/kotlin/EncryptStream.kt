...
/**
 * Result of a single stream drain: [count] bytes were written into
 * the caller's buffer; [finished] reports whether the session output
 * is complete. Partial drains are normal.
 */
data class ReadResult(val count: Int, val finished: Boolean)
 ...
 * the session is live.
 */
class EncryptStream internal constructor(
    /** The Pipeline this session runs against. */
    val parent: Pipeline,
    internal val impl: JEncryptStream,
) : AutoCloseable {
